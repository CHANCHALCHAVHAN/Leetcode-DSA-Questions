...
private:
    ...
    // FIFO queue to store packets
    deque<tuple<int, int, int>> packetQueue;

    // Set to detect duplicates
    unordered_set<string> packetSet;

    // Map from destination to multiset of timestamps for efficient range queries
    unordered_map<int, multiset<int>> destTimeMap;

    // Helper to encode packet uniquely
...
public:
    ...
    bool addPacket(int source, int destination, int timestamp) {
        ...
        if (packetQueue.size() == memoryLimit) {
            ...
            destTimeMap[d].erase(destTimeMap[d].find(t));
        }
    ...
    vector<int> forwardPacket() {
        ...
        packetQueue.pop_front();
        packetSet.erase(encode(source, destination, timestamp));
        destTimeMap[destination].erase(destTimeMap[destination].find(timestamp));
    ...
    int getCount(int destination, int startTime, int endTime) {
        ...
        auto startIt = times.lower_bound(startTime);
        auto endIt = times.upper_bound(endTime);
